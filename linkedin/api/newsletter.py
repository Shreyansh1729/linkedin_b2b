...
def subscribe_to_newsletter(email: str, linkedin: str | None = None) -> bool:
    """
    Subscribe email to LeadPilot newsletter via Brevo form.
...
def ensure_newsletter_subscription(session: AccountSession, linkedin_url: str | None = None):
    """Subscribe the account to the LeadPilot newsletter if enabled."""
    ...
    logger.debug("Subscribing %s to LeadPilot newsletter...", email)
